...
void lotto_winning(int *win_num, int m){//輸入樂透號碼
    printf("請輸入中獎號碼(最多三碼)：");
    do {
            scanf("%d", &win_num[m++]);
            if (m >= 3) break;
...
int main() {
    ...
    if (num_tickets == 0){
        int win_num[3] = {0}, m = 0, lotte_num = 1;
        lotto_winning(win_num, m);//輸入樂透號碼
        printf("\n以下為中獎彩券：\n");
        fseek(rc, 0, SEEK_END);
            ...
            for(int i = 0; i < 5; i++){
                for(int j = 0; j < 7; j ++){
                    for(int k = 0; k < 3; k++){
                        ...
                        else if (win_num[k] == lotte_record_read.lotte_set[i][j]){
                            correct_arr[i] = 1;
                        }
                    }
                }
        ...
        lotte_record.receipt_price = num_tickets * 100;
        ...
        time_t t;
        time(&t);
        lotte_record.receipt_time = ctime(&t);
        fprintf(fp, "= %s =\n", lotte_record.receipt_time);
        for(int j = 1; j <= 5; j++){
            fprintf(fp, "[%d]: ", j);
            if (j > num_tickets){
